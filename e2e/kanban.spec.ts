...
const login = async (page: Page) => {
 ...
 await page.getByLabel('密碼').fill('123');
...
const createBoard = async (page: Page, name: string) => {
 ...
 await page.getByLabel('描述').first().fill(`${name} description`);
...
const createTask = async (page: Page, statusName: string, title: string) => {
 ...
 await expect(statusColumn.getByRole('button', { name: `Task ${title}` })).toBeVisible();
};
...
test.beforeEach(async ({ page }) => {
 await page.addInitScript(() => {
  localStorage.clear();
  localStorage.setItem('kanban-board:e2e', 'true');
 });
});

test('redirects protected board route to login and supports login/logout', async ({ page }) => {
...
test('creates, updates, and deletes a board from the current UI', async ({ page }) => {
 await login(page);
 ...
 await expect(page.getByRole('heading', { level: 2, name: '產品開發 v2' })).toBeVisible();
...
test('creates, edits, and deletes tasks from the current UI', async ({ page }) => {
 await login(page);
...
test('drags a task between board statuses', async ({ browserName, page }) => {
 ...
 await expect(doneColumn.getByRole('button', { name: 'Task 拖曳測試 task' })).toBeVisible();
});

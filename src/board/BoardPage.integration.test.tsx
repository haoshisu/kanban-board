...
const renderBoardPage = () =>
 render(<BoardPage userEmail="you@example.com" userId="local-you@example.com" onLogout={vi.fn()} />)

const readStoredBoards = () => JSON.parse(localStorage.getItem(boardStorageKey) ?? "[]") as Board[]

const readStoredTasks = () => JSON.parse(localStorage.getItem(taskStorageKey) ?? "[]") as Task[]
...
const createTaskFromUi = async (
 ...
 expect(await within(statusColumn).findByLabelText(`Task ${title}`)).toBeVisible()
}
...
describe("BoardPage integration", () => {
 ...
 it("moves a task to another status through the edit form", async () => {
  ...
  expect(await within(inProgressColumn).findByLabelText("Task 設計登入流程")).toBeVisible()
  expect(within(todoColumn).queryByLabelText("Task 設計登入流程")).not.toBeInTheDocument()
 ...
 it("shows only tasks for the selected board when switching boards", async () => {
  ...
  storeBoards([createBoard({ id: "board-1", name: "產品開發" }), createBoard({ id: "board-2", name: "行銷計畫" })])
  storeTasks([
